"""
LLMcheck_railway.py - 軌二部專用版本 (Railway Department Version)

基於 LLMcheck.py 修改，專門處理軌二部的文件格式。

【主要差異】
- 業務部版本（LLMcheck.py）：有條款摘要就處理，不管是否有子項目
- 軌二部版本（本檔案）：如果有子項目就跳過父項目，只處理葉子節點

【使用場景】
軌二部的文件格式：
{
  "項次": "1.1",
  "檢查項目": "給付條件",  ← 這個不處理，但會作為檢索依據
  "子項目": [
    {
      "項次": "1.1.a",
      "檢查項目": "總包價法給付條件"  ← 只處理這個
...
class JSONChecklistQuerySystem:
    ...
    def analyze_full_content(
        ...
        """
        try:
            from core.prompt_templates import get_full_content_extraction_prompt
        except ModuleNotFoundError:
            from prompt_templates import get_full_content_extraction_prompt
        ...
        # 獲取檢查項目
        check_item = item_info['data'].get('檢查項目', item_info['data'].get('主項說明', ''))

        # 使用完整條文提取的 prompt
        prompt = get_full_content_extraction_prompt(
            main_description, parent_item, check_item, clauses_text,
            project_management_checked, design_supervision_checked, user_hint)
...
請直接輸出審查意見，不要包含其他說明文字。"""
    ...
    def process_item(self, item_number, deployment_name="o4-mini",
        ...
        【軌二部專用版本 - Railway Department Version】
        新處理邏輯：
        1. 如果項目有子項目（例如 1.1 有 1.1.a） → 跳過父項目本身，只處理子項目
        2. 如果項目無子項目且有條款摘要 → 處理該項目
        3. 父項目資訊（檢查項目）仍會自動作為子項目的檢索依據（程式碼第740行）

        與業務部版本的差異：
        - 業務部：有條款摘要就處理，不管是否有子項目
        - 軌二部：有子項目就跳過，只處理葉子節點
        """
        ...
        # 【軌二部新邏輯】如果有子項目，跳過當前項目，只處理子項目
        if has_children:
            print(f"【軌二部模式】項次 {item_number} 有子項目，跳過自身處理，只處理子項目...")
        ...
        # 【軌二部新邏輯】如果沒有子項目，一律處理該項目（不管有無條款摘要）
        else:
            print(f"處理當前項目 {item_number}...")

            # 判斷是否有條款摘要格式（用於後續決定輸出格式）
            has_summary_format = current_summary and current_summary.strip() != ""
            if not has_summary_format:
                print(f"⚠ 項次 {item_number} 沒有條款摘要格式，將以完整條文模式輸出")
